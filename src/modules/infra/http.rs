...
impl Validation for HttpsModule {
    fn validate(&self) -> Result<(), crate::validation::ValidationReport> {
        ...
        if self.cert_path.is_empty() {
            report.add_domain_error(crate::validation::ValidationError::new("cert_path", "Certificate path cannot be empty"));
        }
        
        if self.key_path.is_empty() {
            report.add_domain_error(crate::validation::ValidationError::new("key_path", "Key path cannot be empty"));
        }
